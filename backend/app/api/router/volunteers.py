...
from app.services.volunteer_service import delete_volunteer
...
def update_my_profile(update_data: VolunteerUpdate, db: Session = Depends(get_db),
...
@router.delete("/me", response_model=VolunteerRead)
def delete_volunteer_endpoint(
        db: Session = Depends(get_db),
        current_user=Depends(get_current_user)
):
    deleted_volunteer = delete_volunteer(db, volunteer_id=current_user.id)
    if not deleted_volunteer:
